...
class RoutineStore:
    """Persistent conversation-bound local automation scheduler.

    Interval routines remain backward compatible. V0.9 adds one-shot jobs,
    session binding and bounded run counts without granting new permissions.
    """
    ...
    def create(self, name: str, prompt: str, interval_seconds: int, start_in_seconds: int = 0,
        ...
        item = Routine(uuid.uuid4().hex[:12], clean_name, clean_prompt, interval, time.time() + start, True, "never", None,
                       session_id.strip()[:128] or "autonomy", "interval", max_runs, 0)
        self._insert(item)
        return item
    ...
    def mark_result(self, routine_id: str, status: str, now: float | None = None) -> None:
        ...
        next_run = moment + current.interval_seconds
        self.conn.execute(
            "UPDATE routines SET last_status=?, last_run=?, next_run=?, run_count=?, enabled=? WHERE id=?",
            (status, moment, next_run, new_count, 0 if disable else 1, routine_id),
        )
        self.conn.commit()
