...
class SkillStore:
    """Progressive-disclosure local skill library.

    Approved internal skills remain the source of truth. Read-only external
    directories can expose agentskills-style `SKILL.md` documents without
    copying or trusting executable code. Usage evidence is stored locally and
    helps rank relevant skills for future tasks.
    """
    ...
    def __init__(self, path: Path, external_paths: list[Path] | None = None) -> None:
        ...
        self.stats = sqlite3.connect(path.parent / "skill-usage.db", timeout=10)
        self.stats.execute(
            """
    ...
    def _external_files(self) -> dict[str, Path]:
        ...
        for root in self.external_paths:
            try:
                ...
                if direct.is_file():
                    output.setdefault(root.name, direct)
                for file in root.glob("*/SKILL.md"):
                    output.setdefault(file.parent.name, file)
                for file in root.glob("*.md"):
                    output.setdefault(file.stem, file)
            except OSError:
                continue
        return output
    ...
    def read_skill(self, name: str) -> str:
        ...
        found = self._all_files().get(clean)
        if found is None:
            raise FileNotFoundError(f"Unknown skill: {name}")
        path, _source = found
        if path.stat().st_size > 200_000:
            raise ValueError("Skill exceeds 200 KB read limit")
    ...
    def search(self, query: str, limit: int = 5) -> list[SkillMatch]:
        ...
        for name, (path, source) in self._all_files().items():
            try:
                text = path.read_text(encoding="utf-8", errors="replace")[:30_000]
            except OSError:
                continue
            tokens = self._tokens(name + " " + text[:8000])
            overlap = len(q & tokens)
            if not overlap:
                continue
            uses, successes = self._usage(name)
            reliability = (successes / uses) if uses else 0.5
            score = (overlap / max(1, len(q))) + min(0.20, uses * 0.005) + reliability * 0.10
            preview = " ".join(text.split())[:500]
            results.append(SkillMatch(name, score, source, preview, uses))
    ...
    def mark_used(self, name: str, success: bool | None = None) -> None:
        clean = self._normalize(name)
        uses, successes = self._usage(clean)
        new_successes = successes + (1 if success is True else 0)
        self.stats.execute(
            "INSERT OR REPLACE INTO skill_usage(name,uses,successes,last_used) VALUES(?,?,?,?)",
            (clean, uses + 1, new_successes, time.time()),
        )
        self.stats.commit()
    ...
    def context_for(self, query: str, limit: int = 3, max_chars: int = 12_000) -> list[tuple[str, str]]:
        ...
        for match in self.search(query, limit=limit):
            body = self.read_skill(match.name)
            chunk = body[:remaining]
            if not chunk:
                break
            output.append((match.name, chunk))
            self.mark_used(match.name)
            remaining -= len(chunk)
            if remaining <= 0:
                break
        return output
