...
class ChannelBridge:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                if parsed.path == "/health":
                    self._json(200, {"status": "ok", "pending": len(bridge.runtime.channels.pending())})
                    return
    ...
    def _message_json(item) -> dict:
        return {
            "message_id": item.message_id, "direction": item.direction, "channel": item.channel,
    ...
    def _worker(self) -> None:
        while not self.stop_event.is_set():
            for item in self.runtime.channels.pending(limit=20):
                previous = self.runtime.agent.session_id
                denied = False
                ...
                try:
                    ...
                    reply = self.runtime.agent.ask(item.content, deny_sensitive)
                    if denied:
                    ...
                    self.runtime.agent.session_id = previous
            self.stop_event.wait(1.0)

    def run(self) -> None:
        worker = threading.Thread(target=self._worker, daemon=True, name="skynet-channel-worker")
        worker.start()
...
def main() -> None:
    ...
    port = int(os.getenv("SKYNET_CHANNEL_PORT", "8765"))
    ChannelBridge(Path.cwd(), host=host, port=port).run()
